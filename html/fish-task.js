...
if (subject_data.path_id) {
  if (subject_data.path_id.toUpperCase() == "A") {
    ...
    redirect_url = "https://run.pavlovia.org/janetlchang/two-slot-machine-task/" +  "?" + url_params;
    ...
    // fish task is last task, need to redirect to study completion page
      ...
      redirect_url = "https://app.prolific.com/submissions/complete?cc=completioncode"; // completion code for prolific main general study for all groups
  } else {
    redirect_url = "";
...
// var box_loc = [[(- 0.32), 0.25], [0, 0.25], [0.32, 0.25]]; // three fish version
var box_loc = [
  [-0.25, 0.25],
  [0, 0.25],
  [0.32, 0.25],
]; // two fish version, third never used
...
psychoJS.start({
  ...
  resources: [
    ...
    {name: "stimuli/Two-Choice_FishingGame.mp4", path: "stimuli/Two-Choice_FishingGame.mp4"},
    ...
    {name: "stimuli/box_outline.png", path: "stimuli/box_outline.png"},
    {name: "stimuli/clear-jar.PNG", path: "stimuli/clear-jar.PNG"},
    {name: "stimuli/code_component.txt", path: "stimuli/code_component.txt"},
    {name: "stimuli/all_ponds.png", path: "stimuli/all_ponds.png"},
    {name: "stimuli/pond_selector_box.png", path: "stimuli/pond_selector_box.png"},
    {name: "default.png", path: "https://pavlovia.org/assets/default/default.png"},
    // new block and trial assignments
    {name: "conditions/practiceBlockOrder.xlsx", path: "conditions/practiceBlockOrder.xlsx"},
    {name: "conditions/practiceTrials1.xlsx", path: "conditions/practiceTrials1.xlsx"},
    {name: "conditions/practiceTrials2.xlsx", path: "conditions/practiceTrials2.xlsx"},
    {name: "conditions/blockOrderA.xlsx", path: "conditions/blockOrderA.xlsx"},
    {name: "conditions/blockOrderB.xlsx", path: "conditions/blockOrderB.xlsx"},
    {name: "conditions/blockOrderC.xlsx", path: "conditions/blockOrderC.xlsx"},
    {name: "conditions/mainTrials1.xlsx", path: "conditions/mainTrials1.xlsx"},
    {name: "conditions/mainTrials2.xlsx", path: "conditions/mainTrials2.xlsx"},
    {name: "conditions/mainTrials3.xlsx", path: "conditions/mainTrials3.xlsx"},
    {name: "conditions/mainTrials4.xlsx", path: "conditions/mainTrials4.xlsx"},
    {name: "conditions/mainTrials5.xlsx", path: "conditions/mainTrials5.xlsx"},
    {name: "conditions/mainTrials6.xlsx", path: "conditions/mainTrials6.xlsx"},
    {name: "conditions/mainTrials7.xlsx", path: "conditions/mainTrials7.xlsx"},
    {name: "conditions/mainTrials8.xlsx", path: "conditions/mainTrials8.xlsx"},
    {name: "conditions/mainTrials9.xlsx", path: "conditions/mainTrials9.xlsx"},
    {name: "conditions/mainTrials10.xlsx", path: "conditions/mainTrials10.xlsx"},
  ],
});
...
async function experimentInit() {
  ...
  ins_video = new visual.MovieStim({
    ...
    movie: "stimuli/Two-Choice_FishingGame.mp4",
    pos: [0, 0],
    anchor: "center",
    size: [0.9, 0.5],
    ori: 0.0,
  ...
  ins1_image = new visual.ImageStim({
    ...
    name: "ins1_image",
    units: undefined,
    image: "stimuli/instruction_image.png",
    mask: undefined,
    anchor: "center",
    ori: 0.0,
    pos: [0, 0.05],
  ...
  text_3 = new visual.TextStim({
    ...
    name: "text_3",
    text:
      "Press LEFT or RIGHT arrows on your keyboard to select your pond." +
  ...
  text_4 = new visual.TextStim({
    ...
    name: "text_4",
    text:
      "Press LEFT or RIGHT arrows on your keyboard to select your pond. The game takes approximately 10 minutes to complete." +
  ...
  boxB_2 = new visual.Rect({
    win: psychoJS.window,
    name: "boxB_2",
    // width: [0.3, 0.45][0], // three option version
    // height: [0.3, 0.45][1], // three option version
    width: [0.45, 0.5][0], // two option version
    height: [0.45, 0.5][1], // two option version
    ori: 0.0,
    pos: box_loc[0],
    anchor: "center",
    lineWidth: 5.0,
  ...
  Begin_txt = new visual.TextStim({
    ...
    name: "Begin_txt",
    text:
      "You have successfully completed the practice. Now you are ready to start the game." +
  ...
  ExpStart_key = new core.Keyboard({
    psychoJS: psychoJS,
    clock: new util.Clock(),
    waitForStart: true,
  });
  ...
  fish_key_button = new core.Keyboard({
    psychoJS: psychoJS,
    clock: new util.Clock(),
    waitForStart: true,
  });
  ...
  return async function () {
    ...
    if (t >= 0.0 && welcome_key.status === PsychoJS.Status.NOT_STARTED) {
      ...
      psychoJS.window.callOnFlip(function () {
        welcome_key.clock.reset();
      }); // t=0 on next screen flip
      psychoJS.window.callOnFlip(function () {
        welcome_key.start();
      }); // start on screen flip
      psychoJS.window.callOnFlip(function () {
        welcome_key.clearEvents();
      });
    }
    ...
    if (welcome_key.status === PsychoJS.Status.STARTED) {
      ...
      if (_welcome_key_allKeys.length > 0) {
        welcome_key.keys =
          _welcome_key_allKeys[_welcome_key_allKeys.length - 1].name; // just the last key pressed
        welcome_key.rt =
          _welcome_key_allKeys[_welcome_key_allKeys.length - 1].rt;
        welcome_key.duration =
          _welcome_key_allKeys[_welcome_key_allKeys.length - 1].duration;
...
function welcomeRoutineEnd(snapshot) {
  return async function () {
    ...
    if (typeof welcome_key.keys !== "undefined") {
      // we had a response
...
function instruct_videoRoutineEachFrame() {
  return async function () {
    ...
    if (ins_video.status === PsychoJS.Status.FINISHED) {
...
function ins1RoutineEachFrame() {
  return async function () {
    ...
    if (t >= 0.0 && ins1_key.status === PsychoJS.Status.NOT_STARTED) {
      ...
      // keyboard checking is just starting
      psychoJS.window.callOnFlip(function () {
        ins1_key.clock.reset();
      }); // t=0 on next screen flip
      psychoJS.window.callOnFlip(function () {
        ins1_key.start();
      }); // start on screen flip
      psychoJS.window.callOnFlip(function () {
        ins1_key.clearEvents();
      });
    }
    ...
    if (ins1_key.status === PsychoJS.Status.STARTED) {
      ...
      if (_ins1_key_allKeys.length > 0) {
        ...
        ins1_key.duration =
          _ins1_key_allKeys[_ins1_key_allKeys.length - 1].duration;
        // a response ends the routine
        continueRoutine = false;
      }
    }
...
function ins1RoutineEnd(snapshot) {
  return async function () {
    ...
    if (typeof ins1_key.keys !== "undefined") {
      // we had a response
...
function ins2RoutineEachFrame() {
  return async function () {
    ...
    if (t >= 0.0 && ins2_key.status === PsychoJS.Status.NOT_STARTED) {
      ...
      // keyboard checking is just starting
      psychoJS.window.callOnFlip(function () {
        ins2_key.clock.reset();
      }); // t=0 on next screen flip
      psychoJS.window.callOnFlip(function () {
        ins2_key.start();
      }); // start on screen flip
      psychoJS.window.callOnFlip(function () {
        ins2_key.clearEvents();
      });
    }
    ...
    if (ins2_key.status === PsychoJS.Status.STARTED) {
      ...
      if (_ins2_key_allKeys.length > 0) {
        ...
        ins2_key.duration =
          _ins2_key_allKeys[_ins2_key_allKeys.length - 1].duration;
        // a response ends the routine
        continueRoutine = false;
      }
    }
...
function ins3RoutineEachFrame() {
  return async function () {
    ...
    if (t >= 0.0 && ins3_key.status === PsychoJS.Status.NOT_STARTED) {
      ...
      // keyboard checking is just starting
      psychoJS.window.callOnFlip(function () {
        ins3_key.clock.reset();
      }); // t=0 on next screen flip
      psychoJS.window.callOnFlip(function () {
        ins3_key.start();
      }); // start on screen flip
      psychoJS.window.callOnFlip(function () {
        ins3_key.clearEvents();
      });
    }
    ...
    if (ins3_key.status === PsychoJS.Status.STARTED) {
      ...
      if (_ins3_key_allKeys.length > 0) {
        ...
        ins3_key.duration =
          _ins3_key_allKeys[_ins3_key_allKeys.length - 1].duration;
        // a response ends the routine
        continueRoutine = false;
      }
    }
...
function ins4RoutineEachFrame() {
  return async function () {
    ...
    if (t >= 0.0 && ins4_key.status === PsychoJS.Status.NOT_STARTED) {
      ...
      // keyboard checking is just starting
      psychoJS.window.callOnFlip(function () {
        ins4_key.clock.reset();
      }); // t=0 on next screen flip
      psychoJS.window.callOnFlip(function () {
        ins4_key.start();
      }); // start on screen flip
      psychoJS.window.callOnFlip(function () {
        ins4_key.clearEvents();
      });
    }
    ...
    if (ins4_key.status === PsychoJS.Status.STARTED) {
      ...
      if (_ins4_key_allKeys.length > 0) {
        ...
        ins4_key.duration =
          _ins4_key_allKeys[_ins4_key_allKeys.length - 1].duration;
        // a response ends the routine
        continueRoutine = false;
      }
    }
...
function prac_blockLoopBegin(prac_blockLoopScheduler, snapshot) {
  return async function () {
    ...
    pracBlock = new TrialHandler({
      ...
      trialList: "conditions/practiceBlockOrder.xlsx",
      seed: undefined,
      name: "pracBlock",
    });
    ...
    for (const thisBlock of pracBlock) {
      ...
      prac_blockLoopScheduler.add(
        prac_trialsLoopBegin(prac_trialsLoopScheduler, snapshot)
      );
      ...
      prac_blockLoopScheduler.add(
        pracBlockLoopEndIteration(prac_blockLoopScheduler, snapshot)
      );
    }
...
function prac_trialsLoopBegin(prac_trialsLoopScheduler, snapshot) {
  return async function () {
    ...
    for (const thisPrac_trial of prac_trials) {
      ...
      prac_trialsLoopScheduler.add(
        prac_trialsLoopEndIteration(prac_trialsLoopScheduler, snapshot)
      );
    }
...
var block_order_options = [
  "conditions/blockOrderA.xlsx",
  "conditions/blockOrderB.xlsx",
  "conditions/blockOrderC.xlsx",
];
...
function blockLoopBegin(blockLoopScheduler, snapshot) {
  return async function () {
    ...
    for (const thisBlock of block) {
      ...
      blockLoopScheduler.add(
        blockLoopEndIteration(blockLoopScheduler, snapshot)
      );
    }
...
function option_practiceRoutineEachFrame() {
  return async function () {
    ...
    // If user doesn't provide key response for more than 5 seconds, end the trial
    if (t > 5.0 && key_resp_2.getKeys().length === 0) {
      endMsg = "Too slow";
      endTrial = true; // this allows you to skip the "correct/incorrect" msg at end
    ...
    // Check if a key is already down at the start of the trial
    // NOTE: this probably doesn't need to be here since code already does this?
    // if (t === 0 && key_resp_2.getKeys().length > 0) {
    //   // do not register key response, routine needs to continue as if nothing happened
    //   console.log("lift up key to register a response")
    //   endMsg = "Do not hold down a key";
    // }
    
    // Check if a key is held down for more than 2000ms
    // if (key_resp_2.getKeys().filter(key => key.duration > 2000).length > 0) {
    // need to change this to have separate timing variable start once a key starts, and can't really do this
      // register a response for this one but warn them for next time
    //   console.log("Don't hold down key")
    //   endMsg = "Do not hold down key";
    //   endTrial = true; // have to make true in order to allow this endMsg to show, sacrifice the correctness feedback
    // }
    ...
    if (pond_label_2.status === PsychoJS.Status.STARTED) {
    ...
    // Once button is pressed, key_resp_2.status = PsychoJS.Status.STARTED
    ...
    // *key_resp_2* updates
    if (t >= (fish_int + 0.5) && key_resp_2.status === PsychoJS.Status.NOT_STARTED) {
      // if time is more than the waiting interval before fish displayed + 0.5 sec display...  
      // reset status to not started to override Psychopy's default response to key presses
      // key_resp_2.status === PsychoJS.Status.NOT_STARTED
    ...
    // Once PsychoJS.Status.STARTED, check for valid key responses
    if (key_resp_2.status === PsychoJS.Status.STARTED) {
      ...
      if (_key_resp_2_allKeys.length > 0) {
        // collects just the last key pressed, but should be first key only
        key_resp_2.keys = _key_resp_2_allKeys[_key_resp_2_allKeys.length - 1].name; 
...
function option_practiceRoutineEnd(snapshot) {
  return async function () {
    ...
    if (typeof key_resp_2.keys !== "undefined") {
      // we had a response
      psychoJS.experiment.addData("prac_key_resp_rt", key_resp_2.rt + 0.5); // adjust for 0.5 sec offset for early keypress feature
      psychoJS.experiment.addData(
        "prac_key_resp_duration",
        key_resp_2.duration
      );
      routineTimer.reset();
    }
...
function response_practiceRoutineBegin(snapshot) {
  return async function () {
    ...
    frameN = -1;
    continueRoutine = true; // until we're told otherwise
    routineTimer.add(0.8);
    // update component parameters for each repeat
    
    // Button mashing: track total key presses within a one-trial period
    ...
    if (endTrial) {
      ...
      if (
        key_resp_2.keys === pond_p2.toString() ||
        key_resp_2.keys === pond_p3
      ) {
...
function response_practiceRoutineEachFrame() {
  return async function () {
    ...
    frameRemains = 0.0 + 0.8 - psychoJS.window.monitorFramePeriod * 0.75; // most of one frame period left
    if (
      box_outline2_2.status === PsychoJS.Status.STARTED &&
      t >= frameRemains
    ) {
...
function waitRoutineEachFrame() {
  return async function () {
    ...
    if (t >= 0.0 && ExpStart_key.status === PsychoJS.Status.NOT_STARTED) {
      ...
      // keyboard checking is just starting
      psychoJS.window.callOnFlip(function () {
        ExpStart_key.clock.reset();
      }); // t=0 on next screen flip
      psychoJS.window.callOnFlip(function () {
        ExpStart_key.start();
      }); // start on screen flip
      psychoJS.window.callOnFlip(function () {
        ExpStart_key.clearEvents();
      });
    }

    if (ExpStart_key.status === PsychoJS.Status.STARTED) {
      let theseKeys = ExpStart_key.getKeys({
        keyList: ["space"],
        waitRelease: false,
      });
      ...
      if (_ExpStart_key_allKeys.length > 0) {
        ExpStart_key.keys =
          _ExpStart_key_allKeys[_ExpStart_key_allKeys.length - 1].name; // just the last key pressed
        ExpStart_key.rt =
          _ExpStart_key_allKeys[_ExpStart_key_allKeys.length - 1].rt;
        ExpStart_key.duration =
          _ExpStart_key_allKeys[_ExpStart_key_allKeys.length - 1].duration;
        // a response ends the routine
        continueRoutine = false;
      }
    }
...
function option_fishRoutineEachFrame() {
  return async function () {
    ...
    if (pond_label.status === PsychoJS.Status.STARTED) {
      // only update if being drawn
    ...
    if ( t >= (fish_int + 0.5) && fish_key_button.status === PsychoJS.Status.NOT_STARTED) {
      ...
      // keyboard checking is just starting
      psychoJS.window.callOnFlip(function () {
        fish_key_button.clock.reset();
      }); // t=0 on next screen flip
      psychoJS.window.callOnFlip(function () {
        fish_key_button.start();
      }); // start on screen flip
      psychoJS.window.callOnFlip(function () {
        fish_key_button.clearEvents();
      });
    }

    if (fish_key_button.status === PsychoJS.Status.STARTED) {
      let theseKeys = fish_key_button.getKeys({
        keyList: ["left", "right"],
        waitRelease: false,
      });
      ...
      if (_fish_key_button_allKeys.length > 0) {
        fish_key_button.keys =
          _fish_key_button_allKeys[_fish_key_button_allKeys.length - 1].name; // just the last key pressed
        fish_key_button.rt =
          _fish_key_button_allKeys[_fish_key_button_allKeys.length - 1].rt;
        fish_key_button.duration =
          _fish_key_button_allKeys[
            _fish_key_button_allKeys.length - 1
          ].duration;
        // a response ends the routine
        continueRoutine = false;
      }
...
function option_fishRoutineEnd(snapshot) {
  return async function () {
    ...
    psychoJS.experiment.addData("main_trial_keys", fish_key_button.keys);
    if (typeof fish_key_button.keys !== "undefined") {
      // we had a response
      psychoJS.experiment.addData("main_trial_rt", fish_key_button.rt + 0.5); // adjust for 0.5 sec offset for early keypress feature
      psychoJS.experiment.addData(
        "main_trial_duration",
        fish_key_button.duration
      );
...
function response_fishRoutineBegin(snapshot) {
  return async function () {
    ...
    if (endTrial) {
      ...
      if (
        // fish_key_button.keys === pond2.toString() ||
        fish_key_button.keys === pond3 // ReferenceError: pond2 is not defined
      ) {
        nCorr += 1;
...
function block_breakRoutineBegin(snapshot) {
  return async function () {
    ...
    frameN = -1;
    continueRoutine = true; // until we're told otherwise
    routineTimer.add(5.0);
    // update component parameters for each repeat
    // Run 'Begin Routine' code from reward_list
    // block_correct.append(nCorr);

    if (practice_on == false) block_correct.push(nCorr); // skip this if doing a practice round
    psychoJS.experiment.addData("block_correct", block_correct);
...
function block_breakRoutineEachFrame() {
  return async function () {
    ...
    if (
      breakStart_txt.status === PsychoJS.Status.STARTED &&
      t >= frameRemains
    ) {
